...
class FacebookLogin:
    ...
    def __init__(self, uid_phone_mail, password, twofa_code="", machine_id=None, convert_token_to=None, convert_all_tokens=False):
        self.uid_phone_mail = uid_phone_mail
        self.twofa_code = twofa_code.strip()
    ...
    def _handle_2fa(self, error_data):
        if not self.twofa_code:
            return {'success': False, 'error': '2FA code required but not provided'}
        ...
        try:
            data_2fa = {
                ...
                'twofactor_code': self.twofa_code,
            ...
            if 'error' in response_json:
                msg = response_json['error'].get('message', 'Unknown 2FA error')
                if any(x in msg.lower() for x in ["two factor", "code", "invalid", "expired"]):
                    return {'success': False, 'error': f"2FA failed — possibly wrong or expired code ({msg})"}
                return {'success': False, 'error': msg}
           
            return {'success': False, 'error': 'Unknown 2FA response'}
           
        except Exception as e:
            return {'success': False, 'error': f'2FA submission error: {str(e)}'}
...
def main():
    ...
    uid_phone_mail = input(f"{Colors.CYAN}📧 Email / Phone / Username: {Colors.END}").strip()
    ...
    twofa_code = ""
    if has_2fa == 'y':
        twofa_code = input(f"{Colors.CYAN}🔑 Enter current 6-digit 2FA code (from app / SMS / WhatsApp): {Colors.END}").strip()
        if len(twofa_code) != 6 or not twofa_code.isdigit():
            print(f"{Colors.RED}× Code must be exactly 6 digits{Colors.END}")
            return
    ...
    print(f"\n{Colors.YELLOW}🚀 Attempting login...{Colors.END}\n")
    ...
    # Simple one-time retry for wrong/expired 2FA code
    if not result['success'] and any(x in str(result.get('error','')).lower() for x in ["two factor", "code", "invalid", "expired"]):
        print(f"{Colors.YELLOW}⚠️ 2FA code appears incorrect or expired. You can try once more.{Colors.END}")
        new_code = input(f"{Colors.CYAN}🔑 New 6-digit code: {Colors.END}").strip()
        if len(new_code) == 6 and new_code.isdigit():
            fb_login.twofa_code = new_code
            print(f"{Colors.YELLOW}↻ Retrying with new code...{Colors.END}\n")
            result = fb_login.login()
   
    if result['success']:
        print(f"{Colors.GREEN}{Colors.BOLD}")
        print("=" * 80)
        print("          SUCCESS - TOKENS & COOKIES")
        print("=" * 80)
        print(f"{Colors.END}")
       
        print(f"{Colors.CYAN}Original token prefix:{Colors.END} {result['original_token']['token_prefix']}")
        print(f"{result['original_token']['access_token']}\n")
       
        if 'converted_tokens' in result and result['converted_tokens']:
            print(f"{Colors.CYAN}Converted tokens:{Colors.END}")
            for app_key, token_data in result['converted_tokens'].items():
                print(f"  • {app_key:<20} → {token_data['token_prefix']}")
                print(f"     {token_data['access_token']}")
            print()
       
        if result['cookies'].get('string'):
            print(f"{Colors.GREEN}{Colors.BOLD}COOKIES{Colors.END}")
            print("-" * 80)
            print(result['cookies']['string'])
        else:
            print(f"{Colors.YELLOW}No session cookies returned.{Colors.END}")
        ...
        if 'error_user_msg' in result:
            print(f"       {Colors.YELLOW}{result['error_user_msg']}{Colors.END}")
        print()
